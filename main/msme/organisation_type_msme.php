...
<head>
    ...
    <title>Find MSME by Organisation Type</title>
            ...
            <div class="col-12" id='main-col'>
                <div class="grid-container">
                    <?php
                        ...
                        $query = "SELECT DISTINCT organisation_type FROM msme_nic";
                        $result = $conn->query($query);

                        // Fetch the dists and add them to the $dists array
                        while ($row = $result->fetch_assoc()) {
                            $dists[] = $row['organisation_type'];
                        }
                        ...
                        $tableSql = "SELECT udyam_number AS 'Udyam Number', enterpris_name AS 'Enterprise Name', organisation_type AS 'Organisation Type' FROM msme_nic";

                        if (!empty($selectedButtonValue)) {
                            $tableSql .= " WHERE organisation_type = '$selectedButtonValue'";
                        }
                        ...
                        foreach ($tableRecords as $row) {
                            ...
                            echo '<td>' . $row['Organisation Type'] . '</td>';
                            echo '</tr>';
                        }
                        ...
                        if ($tableCurrentPage > 1) {
                            echo '<a href="?table_page=' . $tablePrevPage . '&organisation_type=' . $selectedButtonValue . '" class="btn btn-primary">Previous</a>';
                        }
                        echo '<span style="margin-left:10px"></span>';
                        echo '<a href="?table_page=' . $tableNextPage . '&organisation_type=' . $selectedButtonValue . '" class="btn btn-primary">Next</a>';
                        echo '</div>';
                    ?>
                </div>
            </div>
            <div class="col-12" id='main-col'>
                ...
                <div>
                    <?php
                        // Add dist Pagination
                        if ($distPage > 1) {
                            echo '<a href="?organisation_type_page=' . $distPrevPage . '" class="btn btn-primary">Previous</a>';
                        }
                        echo '<span style="margin-left:10px"></span>';
                        echo '<a href="?organisation_type_page=' . $distNextPage . '" class="btn btn-primary">Next</a>';
                    ?>
                </div>
                <br>
            </div>
