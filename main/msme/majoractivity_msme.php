...
<head>
    ...
    <title>Find MSME by Major Activity</title>
            ...
            <div class="col-12" id='main-col'>
                <div class="grid-container">
                    <?php
                        ...
                        $query = "SELECT DISTINCT majoractivity FROM msme_nic";
                        $result = $conn->query($query);

                        // Fetch the dists and add them to the $dists array
                        while ($row = $result->fetch_assoc()) {
                            $dists[] = $row['majoractivity'];
                        }
                        ...
                        echo '<th>Major Activity</th>';
                        echo '</tr>';
                        echo '</thead>';
                        echo '<tbody>';

                        // Your SQL query for the table
                        $tableSql = "SELECT udyam_number AS 'Udyam Number', enterpris_name AS 'Enterprise Name', majoractivity AS 'Major Activity' FROM msme_nic";

                        if (!empty($selectedButtonValue)) {
                            $tableSql .= " WHERE majoractivity = '$selectedButtonValue'";
                        }
                        ...
                        if ($tableCurrentPage > 1) {
                            echo '<a href="?table_page=' . $tablePrevPage . '&majoractivity=' . $selectedButtonValue . '" class="btn btn-primary">Previous</a>';
                        }
                        echo '<span style="margin-left:10px"></span>';
                        echo '<a href="?table_page=' . $tableNextPage . '&majoractivity=' . $selectedButtonValue . '" class="btn btn-primary">Next</a>';
                        echo '</div>';
                    ?>
                </div>
            </div>
            <div class="col-12" id='main-col'>
                ...
                <div>
                    <?php
                        // Add dist Pagination
                        if ($distPage > 1) {
                            echo '<a href="?majoractivity_page=' . $distPrevPage . '" class="btn btn-primary">Previous</a>';
                        }
                        echo '<span style="margin-left:10px"></span>';
                        echo '<a href="?majoractivity_page=' . $distNextPage . '" class="btn btn-primary">Next</a>';
                    ?>
                </div>
                <br>
            </div>
